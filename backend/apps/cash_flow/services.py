...
def _build_today(company, config: CompanyTaxConfig, today: datetime.date) -> dict:
    ...
    # ZUS — pulled from existing FixedCost model, category=zus_zdrowotne.
    # Only add to obligations when the amount is actually configured (> 0).
    zus_amount = _get_zus_monthly(company, today)
    if zus_amount > _ZERO:
        zus_due_date = _next_due_date(today, config.zus_due_day)
        obligations.append(
            {
                "type": "zus",
                "label": f"ZUS (do {config.zus_due_day}. miesiąca)",
                "amount": float(zus_amount.quantize(_CENT, rounding=ROUND_HALF_UP)),
                "due_date": str(zus_due_date),
                "days_until": (zus_due_date - today).days,
            }
        )

    total_reserved = sum(Decimal(str(o["amount"])) for o in obligations)
...
def _build_month(
    ...
    # --- PIT estimate ---
    pit_estimate = _get_pit_estimate(config, net_revenue + net_b2c, net_costs)

    # --- ZUS ---
    zus_monthly = _get_zus_monthly(company, period_start)
    try:
...
def _get_b2c_revenue(company, period_start, period_end) -> Decimal:
    result = DailyB2CRevenue.objects.filter(
        company=company,
        date__gte=period_start,
        date__lte=period_end,
...
def _get_b2c_net(company, period_start, period_end) -> Decimal:
    """Net B2C revenue (gross minus embedded VAT)."""
    rows = DailyB2CRevenue.objects.filter(
        company=company,
        date__gte=period_start,
        date__lte=period_end,
    )
...
def _get_pit_estimate(
    ...
    """Rough monthly PIT advance estimate.

    This is always an approximation (no amortisation, no other deductions).
    Marked as ``pit_is_estimate=True`` in the response.
    """
    if config.tax_form == CompanyTaxConfig.TAX_FORM_RYCZALT:
        # Ryczałt: tax on gross revenue, no cost deduction
        taxable = max(total_net_revenue, _ZERO)
    else:
        # KPiR linear or scale
...
def _get_expense_breakdown(company, period_start, period_end) -> dict:
    ...
    # 3. Fixed costs — full monthly amount
    fixed = _get_costs_fixed(company, period_end)
    if fixed > _ZERO:
        result["fixed"] = result.get("fixed", _ZERO) + fixed
