...
export function PZCreatePage() {
  ...
  const {
    ...
    queryKey: ['products', 'pz-create', companyId, productSearchDebounced] as const,
    queryFn: ({ pageParam }) =>
    ...
    <div className="min-h-screen bg-background pb-32">
      ...
      <header className="sticky top-0 z-30 border-b border-border/60 bg-background/95 backdrop-blur-xl">
        <div className="mx-auto max-w-3xl px-4 pb-3 pt-10">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h1 className="text-lg font-semibold text-foreground">Nowe PZ</h1>
              <p className="text-[13px] text-muted-foreground">Przyjęcie zewnętrzne</p>
            </div>
          </div>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-3xl space-y-5 px-4 py-5">
        ...
        <section className="rounded-2xl bg-card p-5 shadow-[0_2px_12px_rgba(26,28,31,0.07)]">
          ...
          <div className="space-y-4">
            ...
            {/* supplier */}
            <div>
              <label htmlFor="from_supplier" className="mb-1.5 block text-[13px] font-medium text-muted-foreground">
                Dostawca
              </label>
              <select
                id="from_supplier"
        ...
        <section className="rounded-2xl bg-card p-5 shadow-[0_2px_12px_rgba(26,28,31,0.07)]">
          ...
          <div ref={searchRef} className="relative mb-4">
            ...
            {showProductSearch && (
              <div
                ref={scrollAreaRef}
                className="absolute left-0 right-0 z-50 mt-1 max-h-64 overflow-auto rounded-xl border border-border bg-card shadow-lg"
              >
                {productsLoading && (
          ...
          {lines.length === 0 ? (
            <p className="py-6 text-center text-sm text-muted-foreground">
              Wyszukaj produkty powyżej, aby dodać pozycje do PZ.
            </p>
          ) : (
      ...
      >
        <div className="mx-auto max-w-3xl">
          ...
          >
            {createPz.isPending ? 'Tworzenie PZ…' : `Utwórz PZ (${lines.length} poz.)`}
          </motion.button>
        </div>
      </div>
    </div>
