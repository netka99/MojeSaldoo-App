...
import { customerService, type ImportCustomerError } from '@/services/customer.service';
import { useImportCustomersMutation } from '@/query/use-customers';
...
export function CustomerImportDialog({ onClose }: { onClose: () => void }) {
  ...
  const [preview, setPreview] = useState<{ toCreate: number; toUpdate: number } | null>(null);
  const [errors, setErrors] = useState<ImportCustomerError[]>([]);
  ...
  const handleValidate = async () => {
    ...
    try {
      ...
      setPreview({ toCreate: result.to_create ?? 0, toUpdate: result.to_update ?? 0 });
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      setCommitResult({ created: result.created ?? 0, updated: result.updated ?? 0 });
      setStep('done');
    } catch (e) {
        ...
        <Card className="border-0 bg-surface-card shadow-none">
          <CardHeader className="pb-2">
            ...
            <p className="text-sm text-muted-foreground">
              Wgraj plik CSV lub XLSX z listą klientów.{' '}
              <button
                type="button"
                    ...
                    <>
                      ...
                      >
                        {(() => {
                          ...
                          return `Importuj ${n} ${n === 1 ? 'klienta' : n < 5 ? 'klientów' : 'klientów'}`;
                        })()}
                      </Button>
                    </>
                  )}
